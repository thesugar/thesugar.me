...
const titleFontStyle = {
    font: "bold 100px -apple-system, BlinkMacSystemFont, 'Helvetica Neue', 'Segoe UI', HiraginoCustom, 'Hiragino Kaku Gothic ProN', YuGothic, 'Yu Gothic Medium', Meiryo, sans-serif;",
    lineHeight: 120,
    color: '#121B1C'
};
// 本文部分の文字スタイル
const bodyFontStyle = {
    font: 'bold 80px Palatino, "Hiragino Mincho ProN", "ヒラギノ明朝 Pro W6", "Hiragino Mincho Pro", HGS明朝E, "Yu Mincho", YuMincho, "ＭＳ Ｐ明朝", serif',
    lineHeight: 80,
    color: '#03A9F4'
};
// 画像内側余白
...
const generate = (title, body, fileName) => {
    ...
    for (let index = 0; index < bodyLineCnt; index++) {
        ...
        // タイトル分の高さと余白を加算する
        // ctx.fillText で文字を書く
        ctx.fillText(element, canvasWidth - 'THESUGARME'.length * bodyFontStyle.lineHeight + 20, canvasHeight - bodyFontStyle.lineHeight - padding)
        // ctx.strokeText で文字を縁取り
        ctx.lineWidth = 1.5
        ctx.strokeStyle = "#fff"
        ctx.strokeText(element, canvasWidth - 'THESUGARME'.length * bodyFontStyle.lineHeight + 20, canvasHeight - bodyFontStyle.lineHeight - padding)
    }
...
allPosts.filter(post => !fileNames.includes(post.id + '.jpg')).map(post => {
    generate(
        post.title, post.postedAt, './public/ogp/' + post.id + '.jpg'
    )
})
...
const handleSubmit = (meta: Meta) => {
  ...
  try {
    currentLikedOrNot = JSON.parse(localStorage.getItem(meta.id) as string)
      .liked
  } catch (err) {
    console.log(err)
  }
...
const LikeButton = ({
  ...
  }, [meta.id, setCurrentLiked])

  return (
    <div className="likeArea">
      <div
        className={`heartMark-${currentLiked}`}
        onClick={() => {
          handleSubmit(meta)
      ...
      <style jsx>{`
        ...
        .heartMark-false {
          ...
          box-shadow: inset 0 2px 0px rgba(255, 255, 255, 0.25),
            inset 0 -2px 0px rgba(0, 0, 0, 0.8);
          transition: 0.2s;
        ...
        .heartMark-true {
          ...
          text-align: center;
          font-weight: bold;
          box-shadow: inset 0 2px 0px rgb(255, 120, 187),
            inset 0 -2px 0px rgb(255, 120, 187);
          transition: 0.2s;

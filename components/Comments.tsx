...
export const Comments = ({ id }: { id: string }): JSX.Element => {
  ...
  return (
    <React.Fragment>
      ...
      {currentComments
        ? Array.from(currentComments).map((eachComment: any) => {
            const commentTagsEscaped = eachComment.comment
              .replace(/&/g, '&amp;')
              .replace(/</g, '&lt;')
              .replace(/>/g, '&gt;')
            ...
            const commentWithLink = commentTagsEscaped.replace(
              reg,
              "<a href='$1' target='_blank' style='padding: 0;'>$1</a>"
            )

            return (
              <div
                key={eachComment.postedAt + '-' + eachComment.name}
              ...
              >
                ...
                <div
                  className="comment"
                  dangerouslySetInnerHTML={{ __html: commentWithLink }}
                ></div>
                <style jsx>{`
                  .name-date-container {
                    display: flex;
